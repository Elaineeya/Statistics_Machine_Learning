...
sc = SparkContext(appName="lab3-1-2")
...
# Cache the reusable data (temperature, distance_kernal, date_kernal, time)
cached_temps = filtered_temps.map(lambda x: (float(x[3]),
                                                gaussian_kernel(broadcast_station_dist.value[x[0]], h_distance),
                                                gaussian_kernel(date_diff(x[1], date), h_date),
                                                x[2])).cache()
...
"12:00:00", "10:00:00", "08:00:00", "06:00:00", "04:00:00"]:
    ...
    prediction = cached_temps.map(lambda x: (x[0], x[1], x[2], 
                                     gaussian_kernel(time_diff(x[3],time), h_time)))

    sum_prediction = prediction.map(lambda x: (x[1]+x[2]+x[3], x[0]*(x[1]+x[2]+x[3])))
    sum_prediction = sum_prediction.reduce(lambda a, b: (a[0] + b[0], a[1] + b[1]))

    prod_prediction = prediction.map(lambda x: (x[1]*x[2]*x[3], x[0]*(x[1]*x[2]*x[3])))
    prod_prediction = prod_prediction.reduce(lambda a, b: (a[0] + b[0], a[1] + b[1]))

    sum_predictions[time] = sum_prediction[1]/sum_prediction[0]
    prod_predictions[time] = prod_prediction[1]/prod_prediction[0]
